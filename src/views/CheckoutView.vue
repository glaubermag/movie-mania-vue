<template>
  <div class="min-h-screen bg-cinema-gradient">
    <div v-if="items.length === 0 && !showSuccessModal" class="flex items-center justify-center min-h-screen">
      <div class="text-center">
        ...
        <button @click="goHome" class="button-primary px-6 py-3 font-bold flex items-center justify-center">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
          Voltar às Compras
        </button>
      </div>
    </div>
            ...
            <div class="card-content py-4">
              <div class="space-y-4">
                <div v-for="item in items" :key="item.movie.id" class="flex space-x-3 items-center">
                  <img :src="getImageUrl(item.movie.poster_path)" :alt="item.movie.title" class="w-14 h-20 object-cover rounded shadow-md border border-white/10" />
                  <div class="flex-1">
    ...
    <SuccessModal :isOpen="showSuccessModal" :onClose="handleSuccessModalClose" :orderNumber="orderNumber" />
  </div>
</template>
