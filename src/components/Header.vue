  ...
  <div>
    <header class="sticky top-0 z-50 w-full glass-effect">
      <div class="container mx-auto px-4 py-4">
          ...
          <div class="flex items-center space-x-2 md:space-x-4 ml-2">
            ...
            <form @submit.prevent="handleSearch" class="relative w-32 sm:w-48 md:w-64 hidden md:block">
              <input
                ...
                v-model="searchQuery"
                class="w-full pl-10 bg-white/10 border-white/20 text-white placeholder:text-gray-400 rounded"
              />
              <svg
            ...
            >
              ...
              <span
                ...
                class="absolute -top-2 -right-2 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center animate-pulse-glow"
              >
        ...
        <!-- Menu Mobile -->
        <nav v-if="isMenuOpen" class="md:hidden mt-4 bg-zinc-900 rounded shadow p-4 w-full">
          <form @submit.prevent="handleSearch" class="relative w-full mb-4">
            <input
              type="text"
              placeholder="Buscar filmes..."
              v-model="searchQuery"
          ...
          >
            ...
            <span
              v-if="favoriteMovies.length > 0"
              class="absolute right-4 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center animate-pulse-glow"
            >
              {{ favoriteMovies.length }}
            </span>
          </button>
    ...
    <div
      v-if="showFavorites"
      class="fixed inset-0 z-50 flex items-start justify-end bg-black/30"
      @click.self="showFavorites = false"
    >
      <div
        class="w-full max-w-md bg-zinc-900 rounded-lg shadow-xl mt-24 mr-8 p-6 relative overflow-y-auto max-h-[80vh] border border-zinc-800"
      >
        <button
          @click="showFavorites = false"
          class="absolute top-2 right-2 text-gray-400 hover:text-primary text-2xl font-bold"
        >
          &times;
        </button>
        <h2 class="text-xl font-bold text-white mb-4">Meus Favoritos</h2>
        <div v-if="favoriteMovies.length === 0" class="text-gray-400 text-center py-8">
        ...
        <div v-else class="space-y-4">
          <div
            ...
            :key="movie.id"
            class="flex items-center gap-4 bg-zinc-800 rounded p-2 shadow border border-zinc-700"
          >
            <img
