      ...
      <div class="flex flex-col h-full">
        ...
        <div v-else class="flex-1 overflow-y-auto p-6">
          ...
          <div v-else class="space-y-4">
            <div v-for="item in items" :key="item.movie.id" class="flex space-x-4 bg-white/5 p-4 rounded-lg">
              <img :src="getImageUrl(item.movie.poster_path)" :alt="item.movie.title" class="w-16 h-24 object-cover rounded" />
              <div class="flex-1">
                <h3 class="font-medium text-sm line-clamp-2">{{ item.movie.title }}</h3>
                <p class="text-primary font-semibold mt-1">{{ formatPrice(item.movie.price || 19.99) }}</p>
                <div class="flex items-center justify-between mt-3">
                  <div class="flex items-center space-x-2">
                    <button class="btn-outline w-6 h-6 flex items-center justify-center" @click="updateQuantity(item.movie.id, item.quantity - 1)">
                      <span class="sr-only">Diminuir</span>
                      <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" /></svg>
                    </button>
                  ...
                  <button class="btn-ghost w-6 h-6 flex items-center justify-center text-red-400 hover:text-red-300" @click="removeFromCart(item.movie.id)">
