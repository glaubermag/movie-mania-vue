...
describe('Cart', () => {
  ...
  it('renderiza o carrinho aberto', () => {
    const store = makeStore();
    const wrapper = mount(Cart, { props: { ...props, isOpen: true }, global: { plugins: [store] } });
  ...
  it('exibe loading ao finalizar compra', () => {
    const store = makeStore(cartItems, 40);
    const wrapper = mount(Cart, { props: { ...props, isOpen: true, loading: true }, global: { plugins: [store] } });
